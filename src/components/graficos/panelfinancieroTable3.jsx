...
export default function PanelFinancieroTable3({anio, mes, anioant, mesant, rangomes, data}) {
    ...
    const Tablebody = ({data}) => {
        return (
            <tbody>{
                data?.map((item, idx)  => {
                    return (
                        <tr key={'row-'+ idx} className={`${idx === 2 || idx === 5 || idx === 6 || idx === 11 || idx === 12 || idx === 18 || idx === 19 ? 'bg-[#36a2eb33] font-bold text-black' : idx === 7 || idx === 13 || idx === 18 || idx === 20? ' bg-[#9966ff33] font-bold text-black' : idx === 19 || idx === 22 || idx === 21 || idx === 23 ? 'bg-[#4bc0c033] font-bold text-black' : ''}`}>
                            {
                                item?.map((item, index) => {
                                    return (
