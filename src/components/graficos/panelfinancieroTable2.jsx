    ...
    const Tablebody = ({data}) => {
        return (
            <tbody>{
                data?.map((item, idx)  => {
                    return (
                        <tr key={'row-'+ idx} className={`${idx === 2 || idx === 6 || idx === 7 || idx === 12 || idx === 13 || idx === 19 || idx === 20 ? 'bg-[#36a2eb33] font-bold text-black' : idx === 8 || idx === 14 || idx === 19 || idx === 21? ' bg-[#9966ff33] font-bold text-black' : idx === 20 || idx === 23 || idx === 22 || idx === 24 ? 'bg-[#4bc0c033] font-bold text-black' : ''}`}>
                            {
                                item?.map((item, index) => {
                                    return (
